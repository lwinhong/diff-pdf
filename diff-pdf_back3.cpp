			...
			y++, data1 += stride1, out += stridediff)
		{
			...
			if (s2 == NULL)
			{
				...
				for (int x = 0; x < r1.width * 4; x += 4)
				{
					...
					unsigned char cr2 = *(data1 + x + 0);
					unsigned char cg2 = *(data1 + x + 1);
					unsigned char cb2 = *(data1 + x + 2);
				...
				{
					for (int x = 0; x < (10 < r1.width ? 10 : r1.width) * 4; x += 4)
					{
						*(out + x + 0) = 0;
						*(out + x + 1) = 0;
						*(out + x + 2) = 255;
	...
	Gutter* gutter = NULL)
{
	...
	for (int page = 0; page < pages_total; page++)
	{
		...
		std::string pm = (page1 != NULL ? std::to_string(page) : "NULL") + "-" + (page2 != NULL ? std::to_string(page) : "NULL");
		std::cout << "Progress:" << page + 1 << "," << pages_total << "," << (page_same ? "1" : "0") << "," << pm << std::endl;
